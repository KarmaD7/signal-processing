...
def filter_fir(input):
    cut_freq = 2 * np.pi * 16 / 60
    h_n = (np.sin(cut_freq * (t_split - 8)) / ((t_split - 8) * np.pi))[:17]
    hanning_window = (0.5 + 0.5 * np.cos((2 * np.pi * (t_split - 8)) / 16))[:17]
    h_n[8] = cut_freq / np.pi
    fir_filter = h_n * hanning_window
    return np.convolve(input, fir_filter)[:60]
